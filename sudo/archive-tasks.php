...
if ($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['taskIds'])) {
    ...
    if (count($taskIds) > 0) {
        ...
        $sql = "UPDATE tbltasks SET is_archived = 1 WHERE id IN ($idsString) AND status = 'Completed' AND is_paid = 1 ";
        ...
        if (mysqli_query($con, $sql)) {
            ...
            if (mysqli_affected_rows($con) > 0) {
                $_SESSION['alert'] = '<div class="alert alert-success border-0 d-flex align-items-center" role="alert">
                                        ...
                                        <p class="mb-0 flex-1">Tasks archived successfully!</p>
                ...
                $_SESSION['alert'] = '<div class="alert alert-warning border-0 d-flex align-items-center" role="alert">
                                        ...
                                        <p class="mb-0 flex-1">No tasks were archived. Please ensure the tasks are completed and paid!</p>
                                        <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
                                      </div>';
            }
        } else {
            $_SESSION['alert'] = '<div class="alert alert-danger border-0 d-flex align-items-center" role="alert">
                                      ...
                                      <p class="mb-0 flex-1">Error archiving tasks: ' . mysqli_error($con) . '</p>
                                      <button class="btn-close" type="button" data-bs-dismiss="alert" aria-label="Close"></button>
                                  </div>';
        }
    } else {
